...
GO
CREATE procedure [dbo].[create_season_team_min_max_player_stats] as


-- drop table if exists
if object_id('dbo.season_team_min_max_player_stats', 'U') is not null
drop table dbo.season_team_min_max_player_stats; 
...
select 
    ...
    ,sum([field_goals_made]                             ) as sum_field_goals_made
    ,sum([field_goals_attempted]                        ) as sum_field_goals_attempted
    ,sum([free_throws_made]                             ) as sum_free_throws_made
    ,sum([free_throws_attempted]                        ) as sum_free_throws_attempted
    ,sum([three_pointers_made]                          ) as sum_three_pointers_made
    ,sum([three_pointers_attempted]                     ) as sum_three_pointers_attempted
    ,sum([offensive_rebounds]                           ) as sum_offensive_rebounds
    ,sum([defensive_rebounds]                           ) as sum_defensive_rebounds
    ,sum([assists]                                      ) as sum_assists
    ,sum([personal_fouls]                               ) as sum_personal_fouls
    ,sum([turnovers]                                    ) as sum_turnovers
    ,sum([blocks]                                       ) as sum_blocks
    ,sum([plus_minus]                                   ) as sum_plus_minus
    ,sum([rebounds]                                     ) as sum_rebounds
    ,sum([pra]                                          ) as sum_pra
    ,sum([astocs]                                       ) as sum_astocs
    ,sum([minutes_rank]                                 ) as sum_minutes_rank
    ,sum([points_rank]                                  ) as sum_points_rank
    ,sum([field_goals_made_rank]                        ) as sum_field_goals_made_rank
    ,sum([field_goals_attempted_rank]                   ) as sum_field_goals_attempted_rank
    ,sum([free_throws_made_rank]                        ) as sum_free_throws_made_rank
    ,sum([free_throws_attempted_rank]                   ) as sum_free_throws_attempted_rank
    ,sum([three_pointers_made_rank]                     ) as sum_three_pointers_made_rank
    ,sum([three_pointers_attempted_rank]                ) as sum_three_pointers_attempted_rank
    ,sum([offensive_rebounds_rank]                      ) as sum_offensive_rebounds_rank
    ,sum([defensive_rebounds_rank]                      ) as sum_defensive_rebounds_rank
    ,sum([assists_rank]                                 ) as sum_assists_rank
    ,sum([personal_fouls_rank]                          ) as sum_personal_fouls_rank
    ,sum([turnovers_rank]                               ) as sum_turnovers_rank
    ,sum([blocks_rank]                                  ) as sum_blocks_rank
    ,sum([plus_minus_rank]                              ) as sum_plus_minus_rank
    ,sum([rebounds_rank]                                ) as sum_rebounds_rank
    ,sum([pra_rank]                                     ) as sum_pra_rank
    ,sum([astocs_rank]									) as sum_astocs_rank
from [dbo].[player_primary_team_season_rank]
group by
	[season]
    , [primary_team_id]
)
...
into dbo.season_team_min_max_player_stats
from t00
GO
